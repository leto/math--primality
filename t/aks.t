...
my $smallest_r = Math::Primality::AKS::_find_smallest_r(Math::GMPz->new(4099)); 
is("$smallest_r", "691", "smallest r for 4099 should be 691");
$smallest_r = Math::Primality::AKS::_find_smallest_r(Math::GMPz->new(10007)); 
...
for (my $i = 1; $i < 7; $i++) {
  my $n = $i * 100;
  my $a = ceil(log2($n));
  my $b = Math::Primality::AKS::_Rmpz_logbase2cl(Math::GMPz->new($n));
...
for (my $i = 1; $i < 7; $i++) {
  my $n = $i * 100;
  $a = floor(log2($n));
  $b = Math::Primality::AKS::_Rmpz_logbase2fl(Math::GMPz->new($n));
  is("$a", "$b", "log2($n) == _Rmpz_logbase2fl($n)");
}
### check _calculate_max_a ###
my $max_a = Math::Primality::AKS::_calculate_max_a(Math::GMPz->new(797), Math::GMPz->new(10007));
is("$max_a", "789", "max a of 10007 (with r of 797) should be 789");
$max_a = Math::Primality::AKS::_calculate_max_a(Math::GMPz->new(691), Math::GMPz->new(4099));
